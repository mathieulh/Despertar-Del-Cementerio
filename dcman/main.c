...
int dcIdStorageCreateAtomicLeaves(u16 *leaves, int n)
{
	...
	int res = sceIdStorage_driver_99ACCB71(leaves, n);
...
/*static void WriteFile(char *file, void *buf, int size)
{
...
int OnModuleStart(SceModule2 *mod)
{
	...
	{
		MAKE_CALL(mod->text_addr+0x8914, WriteBlockPatched);
		WriteBlock = (void *)(mod->text_addr+0x8E5C);
		ClearCaches();
	}

	else if (strcmp(mod->modname, "sceWlan_Driver") == 0)
	{
		MAKE_CALL(mod->text_addr+0x4DAC, WlanFuncPatched);
		WlanFunc = (void *)(mod->text_addr+0xD42C);
		ClearCaches();
	}
